---
description: "Apply the unity-2d sub-domain assets, project project data, then adapt"
requires: [selected_domain, selected_subdomain, merge_mode]
---

# Stage 7 — Apply and adapt the Unity 2D system
...
5. **Call `@system-builder` only if** the project already had a system and `merge_mode` is
   `extend`, to wire routing from the existing orchestrator into the Unity 2D orchestrator.
